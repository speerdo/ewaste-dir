-- Optional logo/image URL when approving a claim (recycling_centers.logo + verified_partners.logo_url).
-- Drop single-arg overload so approve_claim(claim_id) resolves to the two-arg form (default p_image_url).

DROP FUNCTION IF EXISTS approve_claim(uuid);

CREATE OR REPLACE FUNCTION approve_claim(p_claim_id uuid, p_image_url text DEFAULT NULL)
RETURNS TEXT
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  zip5 := substring(trim(claim_rec.zip) from '^\d{5}');
  image_trim := NULLIF(trim(COALESCE(p_image_url, '')), '');
  ...
  IF claim_rec.submission_type = 'update' THEN
    ...
    ELSE
      INSERT INTO recycling_centers (
        ...
        description, logo, accepted_items, services_offered, certifications,
        ...
        claim_rec.city,
        claim_rec.state,
        CASE WHEN zip5 IS NOT NULL THEN zip5::integer ELSE NULL END,
  ...
  IF claim_rec.tier IN ('premium', 'featured') THEN
    ...
    IF vp_id IS NOT NULL THEN
      UPDATE verified_partners SET
        ...
        logo_url         = COALESCE(image_trim, logo_url),
    ...
    ELSE
      INSERT INTO verified_partners (
        ...
        description, logo_url, accepted_devices, certifications, hours,
        tier, is_featured,
